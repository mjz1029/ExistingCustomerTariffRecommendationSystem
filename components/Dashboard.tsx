...
const Dashboard: React.FC<DashboardProps> = ({ results, onViewDetail }) => {
  ...
  const stats = useMemo(() => {
    ...
    // Removed totalSave calculation as requested
    const riskHigh = results.filter(r => r.riskLevel === 'high').length;
    const upgradeCount = results.filter(r => r.recommendedPlan.price > r.user.currentPrice).length;
    ...
    return { total, riskHigh, upgradeCount, distData };
  }, [results]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-white p-4 rounded-lg shadow-sm border border-slate-200">
          <p className="text-sm text-slate-500">消费升级潜力</p>
          <p className="text-2xl font-bold text-brand-600">{stats?.upgradeCount} <span className="text-sm font-normal text-slate-400">户</span></p>
        </div>
      </div>
